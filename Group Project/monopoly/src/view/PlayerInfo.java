...
/**
 * 
 * Record a player panel for display at the end of the game
 * 
...
public class PlayerInfo extends JPanel {
	...
	public void paint(Graphics g) {
		this.setOpaque(false); // background transparent
	...
	private void drawPlayers(Graphics g) {
		...
		if (GameRunning.day >= GameRunning.GAME_DAY) {
			str ="reach game max day "+GameRunning.GAME_DAY;
		}
		//max money
		PlayerModel p1 = players.get(0);
		PlayerModel p2 = players.get(1);
		if (GameRunning.MONEY_MAX > 0 && p1.getCash() >= GameRunning.MONEY_MAX) {
			str ="\"" + p1.getName() +"\" money reach maximunm money.";
		} else if (GameRunning.MONEY_MAX > 0 && p2.getCash() >= GameRunning.MONEY_MAX) {
			str ="\"" + p2.getName() +"\" money reach maximunm money";
		}
		// break down
		if (p1.getCash() < 0 ){
			str ="\"" + p1.getName() +"\"break down.";
		} else if (p2.getCash() < 0 ){
			str ="\"" + p2.getName() +"\"break down.";
		}
		FontMetrics fm = g.getFontMetrics();
		g.drawString("over reason£º"+str, 200 - fm.stringWidth(str)/2, 86);
	}
